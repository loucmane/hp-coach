        ...
        "properties": {
            ...
            "steps": {
                ...
                "description": (
                    ...
                    "from zero math background. Step 1 is ALWAYS 'understand "
                    "the problem' (restate in plain language). The final "
                    "step is ALWAYS the verdict + why it matters. Aim for "
                    "10+ steps in quant (XYZ / KVA / NOG), 4-6 in verbal "
                    "(MEK / LÄS), 2-3 in ORD. "
                    "\n\nPhase A.6V — tier each step as either 'essential' "
                    "(the moves a student needs to follow the strategy) or "
                    "'detail' (vocabulary definitions, first-principles "
                    "re-explainers, intermediate clarifications, "
                    "verifications). The SPA renders essentials in full by "
                    "default and collapses details to title-only previews; "
                    "a 'Jag förstår fortfarande inte' button reveals all "
                    "details inline. Tier liberally: when in doubt, mark "
                    "as 'detail' so the default view stays scannable."
                ),
                "items": {
                    "type": "object",
                    "properties": {
                        ...
                        "tier": {
                            "type": "string",
                            "enum": ["essential", "detail"],
                            "description": (
                                "'essential' = core moves the student needs "
                                "to follow the strategy (problem framing, "
                                "strategy choice, each algebra move, "
                                "conclusion). 'detail' = vocabulary "
                                "definitions ('kvadrera betyder…'), "
                                "first-principles re-explainers of basic "
                                "operations, intermediate clarifications "
                                "between two essential moves, verification "
                                "steps. The SPA renders essentials in full "
                                "by default; details are collapsed previews "
...
def validate_explanation(payload: dict) -> list[str]:
    ...
    if not isinstance(steps, list) or not steps:
        ...
        for i, s in enumerate(steps):
            ...
            if "tier" in s and s["tier"] not in ("essential", "detail"):
                errors.append(f"steps[{i}].tier must be 'essential' or 'detail'")
