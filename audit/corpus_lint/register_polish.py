...
def merge() -> None:
    ...
    for pf in polish_files:
        ...
        # LÄS uses an umlaut in qids so we accept both `läs` and ASCII `las`
        # for the section suffix.
        m = re.search(r'-(mek|läs|las|elf|ord|xyz|kva|nog|dtk)$', pf.stem, re.IGNORECASE)
        if not m:
